...
import type { StripeLogger } from './stripe.logger';
...
export class StripeService {
  ...
  async updateCustomer(
    ...
    try {
      const customer = await this.stripe.customers.update(customerId, {
        ...
        address: this.addressFromDto(dto.address),
        invoice_prefix: dto.invoicePrefix,
        invoice_settings: dto.defaultPaymentMethod
  ...
  async createPaymentMethod(
    ...
    try {
      let card: Stripe.PaymentMethodCreateParams.Card = dto.card as Card2Dto;
      if ((dto.card as Card1Dto)?.number) {
        ...
        } as Stripe.PaymentMethodCreateParams.Card;
      }
      const payload = {
        ...
        billing_details: dto.billingDetails
          ? {
              address: {
                ...dto.billingDetails.address,
                postal_code: dto.billingDetails.address.postalCode,
        ...
        radar_options: dto.radar_session
          ? { session: dto.radar_session }
          : undefined,
        ...
        us_bank_account: dto.usBankAccount
          ? {
              ...
              financial_connections_account:
                dto.usBankAccount.financialConnectionsAccount,
    ...
    try {
      const payload = {
        ...
        product_data: dto.productData
          ? {
              active:
                dto.productData.active !== undefined
                  ? dto.productData.active
                  : true,
        ...
        recurring: dto.recurring
          ? {
              ...
              interval_count:
                dto.recurring.intervalCount == null
                  ? undefined
                  : dto.recurring.intervalCount,
              trial_period_days:
                dto.recurring.trialPeriodDays == null
                  ? undefined
                  : dto.recurring.trialPeriodDays,
          ...
          ? {
              ...
              recurring: dto.defaultPriceData.recurring
                ? {
                    ...
                    interval_count:
                      dto.defaultPriceData.recurring.intervalCount,
                  }
                : undefined,
        ...
        billing_thresholds: dto.billingThresholds
          ? {
              ...
              reset_billing_cycle_anchor:
                dto.billingThresholds.resetBillingCycleAnchor,
            }
          : undefined,
        ...
        payment_settings: dto.paymentSettings
          ? {
                ...
                ? {
                    card: dto.paymentSettings.paymentMethodOptions.card
                      ? {
                          mandate_options: {
                            amount:
                              dto.paymentSettings.paymentMethodOptions.card
                                .mandateOptions.amount,
                            amount_type:
                              dto.paymentSettings.paymentMethodOptions.card
                                .mandateOptions.amountType,
                            description:
                              dto.paymentSettings.paymentMethodOptions.card
                                .mandateOptions.description,
                          },
                    ...
                    us_bank_account: {
                      financial_connections: {
                        permissions:
                          dto.paymentSettings.paymentMethodOptions.usBankAccount
              ...
              save_default_payment_method:
                dto.paymentSettings.saveDefaultPaymentMethod,
            }
          : undefined,
        promotion_code: dto.promotionCode,
    ...
    try {
      const subscription = await this.stripe.subscriptions.update(
        subscriptionId,
        {
                ...
                payment_method_options: dto.paymentSettings.paymentMethodOptions
                  ? {
                      ...
                      us_bank_account: {
                        financial_connections: {
                          permissions:
                            dto.paymentSettings.paymentMethodOptions
                              .usBankAccount?.financialConnections,
                        },
                        verification_method:
                          dto.paymentSettings.paymentMethodOptions.usBankAccount
                            ?.verificationMethod,
          ...
          proration_date: dto.prorationDate
            ? Math.floor(dto.prorationDate.valueOf() / 1000)
            : undefined,
          expand: ['latest_invoice.payment_intent'],
        }
      );
  ...
  async updateDefaultSubscriptionPaymentMethodFromPaymentIntent(
    subscriptionId: string,
    ...
    try {
      ...
      return this.handleError(
        exception,
        'Update Default Subscription Payment Method From PaymentIntent'
      );
    }
  }
  ...
  async listUsageRecordSummaries(
    ...
    try {
      const usageRecordSummaries =
        await this.stripe.subscriptionItems.listUsageRecordSummaries(
          subscriptionItemId,
          {
            limit: params?.limit,
            starting_after: params?.startingAfter,
  ...
  async upcomingInvoicePreview(
    ...
    try {
      ...
      const subscription_items = dto.prices?.map(
        (i) =>
          ({
            id:
              i.id ||
              subscription.items.data.find((x) => x.price.id === i.priceId)?.id,
            price: i.priceId,
            quantity: i.quantity,
          } as Stripe.InvoiceRetrieveUpcomingParams.SubscriptionItem)
      );
      const invoice_items = dto.prices?.map(
        (i) =>
          ({
            invoiceitem: i.id,
            price: i.priceId,
      ...
      const invoice = await this.stripe.invoices.retrieveUpcoming({
        ...
        subscription_proration_date: dto.prorationDate
          ? Math.floor(dto.prorationDate.valueOf() / 1000)
  ...
  async createInvoice(
    ...
    try {
      const invoice = await this.stripe.invoices.create({
        ...
        automatic_tax:
          dto.automaticTax != null ? { enabled: dto.automaticTax } : undefined,
        collection_method: dto.collectionMethod,
  ...
  ): Promise<BaseDataResponse<InvoiceItemDto>> {
    try {
  ...
  async createQuote(dto: SaveQuoteDto): Promise<SaveQuoteResponse> {
    try {
      ...
      const quote = await this.stripe.quotes.create({
        ...
        application_fee_percent: dto.applicationFeePercent,
        automatic_tax:
          dto.automaticTaxEnabled !== undefined
            ? { enabled: dto.automaticTaxEnabled }
            : undefined,
        collection_method: dto.collectionMethod,
        customer: dto.customer,
        default_tax_rates: dto.defaultTaxRates,
        ...
        invoice_settings: dto.invoiceSettings
          ? { days_until_due: dto.invoiceSettings.daysUntilDue }
          : undefined,
        line_items: lineItems,
        metadata: dto.metadata,
  ...
  async finalizeQuote(
    ...
    try {
      const opts: Stripe.QuoteFinalizeQuoteParams = expiresAt
  ...
  async getSubscriptionScheduleById(
    scheduleId: string
  ...
  async releaseSubscriptionSchedule(
    ...
    try {
      const schedule = await this.stripe.subscriptionSchedules.release(
        scheduleId
      );
  ...
  async createSubscriptionSchedule(
    dto: CreateSubscriptionScheduleDto
  ): Promise<BaseDataResponse<SubscriptionScheduleDto>> {
    try {
  ...
  async updateSubscriptionSchedule(
    id: string,
    dto: UpdateSubscriptionScheduleDto
  ): Promise<BaseDataResponse<SubscriptionScheduleDto>> {
    try {
  ...
  async getTestClockList(
    ...
    try {
      ...
      return {
        success: true,
        data: testClocks.data.map((testClock) =>
          this.testClockToDto(testClock)
        ),
  ...
  private getIdOrDtoFieldValue<T, Q>(
    stripeValue: Q | null | string,
    convert: (Q) => T
  ): T | null | string {
        ...
        ? {
            creditedItems: item.proration_details.credited_items
              ? {
                  ...
                  invoiceLineItems:
                    item.proration_details.credited_items.invoice_line_items,
                }
              : undefined,
          }
        : null,
  ...
  private invoiceItemToDto(item: Stripe.InvoiceItem): InvoiceItemDto {
    ...
    return {
      ...
      discounts: item.discounts
        ?.map((d) => d as DiscountDto)
        ?.filter((d) => d?.coupon || d?.discount),
      liveMode: item.livemode,
      metadata: item.metadata,
  ...
  private invoiceToDto(invoice: Stripe.Invoice): InvoiceDto {
    const subscription = this.getIdOrDtoFieldValue(invoice.subscription, (v) =>
      this.subscriptionToDto(v)
    );
    const quote = this.getIdOrDtoFieldValue(invoice.quote, (v) =>
      this.quoteToDto(v)
    );
    return {
      id: invoice.id,
      ...
      paymentSettings: invoice.payment_settings
        ? {
            paymentMethodOptions: {
              acssDebit:
                invoice.payment_settings.payment_method_options?.acss_debit,
              bancontact:
                invoice.payment_settings.payment_method_options?.bancontact,
  ...
  private priceToDto(price: Stripe.Price): PriceDto {
    const product = this.getIdOrDtoFieldValue(price.product, (v) =>
      this.productToDto(v)
    );
    return {
      id: price.id,
  ...
  private productToDto(product: Stripe.Product): ProductDto {
    const defaultPrice = this.getIdOrDtoFieldValue(product.default_price, (v) =>
      this.priceToDto(v)
    );
    return {
      id: product.id,
  ...
  private planToDto(plan: Stripe.Plan): PlanDto {
    const product = this.getIdOrDtoFieldValue(plan.product, (v) =>
      this.productToDto(v)
    );
    return {
      id: plan.id,
  ...
  private subscriptionScheduleToDto(
    item: Stripe.SubscriptionSchedule
  ): SubscriptionScheduleDto {
    return {
      id: item.id,
      object: item.object,
      ...
      shipping: pi.shipping
        ? {
            address: pi.shipping.address
              ? this.addressToDto(pi.shipping.address)
              : null,
    ...
    return {
      ...
      card: pm.card
        ? {
            ...
            threeDSecureUsageSupported: pm.card.three_d_secure_usage
            ...
            wallet: pm.card.wallet
              ? {
                  ...
                  masterPass: pm.card.wallet.masterpass
                    ? {
                        billingAddress: this.addressToDto(
                          pm.card.wallet.masterpass.billing_address
                        ),
                        email: pm.card.wallet.masterpass.email,
                        name: pm.card.wallet.masterpass.name,
                        shippingAddress: this.addressToDto(
                          pm.card.wallet.masterpass.shipping_address
                        ),
                      }
                    : null,
                  ...
                  visaCheckout: pm.card.wallet.visa_checkout
                    ? {
                        billingAddress: this.addressToDto(
                          pm.card.wallet.visa_checkout.billing_address
                        ),
                        email: pm.card.wallet.visa_checkout.email,
                        name: pm.card.wallet.visa_checkout.name,
                        shippingAddress: this.addressToDto(
                          pm.card.wallet.visa_checkout.shipping_address
                        ),
                      }
                    : null,
                }
        ...
        ? {
            ...
            generatedFrom: pm.sepa_debit.generated_from
              ? {
                  ...
                  setupAttempt: pm.sepa_debit.generated_from
                    .setup_attempt as string,
                }
              : null,
